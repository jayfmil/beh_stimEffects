function [results, act,pos] = apply_by_index(f, index, dim, ...
                                                  matrices, varargin)
...
for identifier=uniq_index'
  ...
  [result_for_index,act_for_index,pos_for_index] = f(submatrices{:}, varargin{:});
  results = cat(dim, results, result_for_index);
  act = cat(dim,act,act_for_index);
  pos = cat(dim,pos,pos_for_index);
