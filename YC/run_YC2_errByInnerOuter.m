function run_YC2_errByInnerOuter(subjs,figDir)
%
...
distErr_innerOuter_stim    = NaN(length(subjs),2,5);
distErr_innerOuter_nonStim = NaN(length(subjs),2,5);
...
for s = 1:length(subjs)
    ...
    [distErr_inner_stim,distErr_outer_stim] = YC_innerOuter_err(events,stim_mask);
    distErr_innerOuter_stim(s,1,1) = distErr_inner_stim;
    distErr_innerOuter_stim(s,2,1) = distErr_outer_stim;
    
    % and all nonstim
    stim_mask = [events.isStim];
    [distErr_inner_nonStim,distErr_outer_nonStim] = YC_innerOuter_err(events,stim_mask==0);
    distErr_innerOuter_nonStim(s,1,1) = distErr_inner_nonStim;
    distErr_innerOuter_nonStim(s,2,1) = distErr_outer_nonStim;
    ...
    for roi = 1:4
        ...
        if any(eventInds{roi});
            ...
            [distErr_inner_stim,distErr_outer_stim] = YC_innerOuter_err(eventsROI,stim_mask);
            distErr_innerOuter_stim(s,1,roi+1) = distErr_inner_stim;
            distErr_innerOuter_stim(s,2,roi+1) = distErr_outer_stim;
            
            % and all nonstim in region            
            [distErr_inner_nonStim,distErr_outer_nonStim] = YC_innerOuter_err(eventsROI,stim_mask==0);
            distErr_innerOuter_nonStim(s,1,roi+1) = distErr_inner_nonStim;
            distErr_innerOuter_nonStim(s,2,roi+1) = distErr_outer_nonStim;
...
for r = 1:length(regions)
    ...
    % Figure 1. difference between outer and inner environemtn for stim and non stim
    stim_data    = distErr_innerOuter_stim(:,:,r);
    nonStim_data = distErr_innerOuter_nonStim(:,:,r);
    
    innerDiff = [diff(stim_data,[],2) diff(nonStim_data,[],2)];
    m = nanmean(innerDiff,1);
    s = nanstd(innerDiff);
    e = s./sqrt(sum(~isnan(innerDiff))-1);
    figure(1)
    clf
    ...
    ylabel('Outer - Inner (Raw)','fontsize',20);
    set(gca,'fontsize',20);
    grid on
    set(gca,'gridlinestyle',':')
    title(regions{r});
    fname = fullfile(figSubDir,'beh_byInner');
    print('-depsc2','-loose',fname);
    figs(r).beh_byInner = fname;
    
    [h,p,c,s] = ttest(innerDiff);
    
    % Figure 1. difference between outer and inner environemtn for stim and
    % non stim percent cahnge
    innerDiff = [diff(stim_data,[],2)./stim_data(:,1) diff(nonStim_data,[],2)./nonStim_data(:,1)]*100;
    m = nanmean(innerDiff,1);
    s = nanstd(innerDiff);
    e = s./sqrt(sum(~isnan(innerDiff))-1);
    figure(2)
    ...
    set(gca,'xticklabel',{'Stim', 'Non-stim'});
    ylabel('Outer - Inner (%)','fontsize',20);
    set(gca,'fontsize',20);
    grid on
    set(gca,'gridlinestyle',':')
    title(regions{r});
    fname = fullfile(figSubDir,'beh_byInner_percent');
    print('-depsc2','-loose',fname);
    figs(r).beh_byInner_percent = fname;
    ...
    set(gca,'xticklabel',{'Inner', 'Outer'});
    ylabel('Raw change due to stim','fontsize',20);
    set(gca,'fontsize',20);
    grid on
    set(gca,'gridlinestyle',':')
    title(regions{r});
    fname = fullfile(figSubDir,'stimEff_byInner');
    print('-depsc2','-loose',fname);
    figs(r).stimEff_byInner = fname;
    ...
    set(gca,'gridlinestyle',':')
    title(regions{r});
    fname = fullfile(figSubDir,'stimEff_byInner_percent');
    print('-depsc2','-loose',fname);
    figs(r).stimEff_byInner_percent = fname;    
end

%%%% make report
texName = 'YC2_errByInnerOuter.tex';
...
for r = 1:length(figs)
    ...
    fprintf(fid,'\\subfigure[]{{\\includegraphics[width=0.49\\textwidth]{%s}}}\n',figs(r).beh_byInner_percent);
    fprintf(fid,'\\subfigure[]{{\\includegraphics[width=0.49\\textwidth]{%s}}}\n',figs(r).stimEff_byInner);
    fprintf(fid,'\\subfigure[]{{\\includegraphics[width=0.49\\textwidth]{%s}}}\n',figs(r).stimEff_byInner_percent);
    fprintf(fid,['\\caption{\\textbf{%d subjects.  }\\textbf{A: } Raw increase in accuracy for objects in outer region. ',...
                 '\\textbf{B: } Percent increase in accuracy for objects in outer region ',...
